...
class MPAPI_GPX:
	...
	def getMP_URL(self,mp_URL_base,mp_command,mp_URL_email,mp_private_key):
		return str.format("{0}/{1}?email={2}&key={3}",mp_URL_base,mp_command,mp_URL_email,mp_private_key)

	#gets a profile
	def getMP_Profile(self,mp_URL_email,mp_private_key):
		#conditional load of the user profile
		profile_url = getMP_URL(self.mp_URL_base,'get-user',mp_URL_email,mp_private_key)
		return getMP_API(profile_url)

	#returns a string of XML 
	def getMP_GPX(self,mp_URL_email,mp_private_key):
		todos_url = self.getMP_URL(self.mp_URL_base,'get-to-dos',mp_URL_email,mp_private_key)
		...
		if len(route_ids) > 0:		
			#return routes
			routes_url = self.getMP_URL(self.mp_URL_base,'get-routes',mp_URL_email,mp_private_key)
